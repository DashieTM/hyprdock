...
const DEFAULT_CONFIG: Lazy<OptionalHyprDock> = Lazy::new(|| OptionalHyprDock {
    ...
    get_monitors_command: Some(HyprdockCommand::new("hyprctl", &["monitors", "all"])),
...
impl HyprDock {
    ...
    pub fn handle_close(&self) {
        ...
        if self.has_external_monitor() {
            ...
            self.wallpaper();
            self.reload_bar();
            save_hypr_monitor_data(self.monitor_config_path.clone(), None, Some(monitor_hash));
        } else {
            self.utility();
            self.lock_system();
            save_hypr_monitor_data(self.monitor_config_path.clone(), None, Some(monitor_hash));
